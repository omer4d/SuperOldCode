...
/*
	...
	int lModelCounter;
    for
		(lModelCounter = 0; lModelCounter < pMap.mModels.size(); ++lModelCounter)
	{
  ...
  */
	...
	fprintf(pFile, "%d ", pMap.mVertices.size());
	for
		(int lVertexCounter = 0; lVertexCounter < pMap.mVertices.size(); ++lVertexCounter)
	{
  fprintf(pFile, "%f %f %f ", pMap.mVertices[lVertexCounter].mPosition[0], pMap.mVertices[lVertexCounter].mPosition[1], pMap.mVertices[lVertexCounter].mPosition[2]);
        /*
...
*/
    ...
    for(i = 0; i < pMap.mFaces.size(); i++)
     tnum += (pMap.mFaces[i].mNbMeshVertices / 3);

	//fprintf(pFile, "********* Face Lump *********\n");
	fprintf(pFile, "%d ", tnum);

    for
		(int lFaceCounter = 0; lFaceCounter < pMap.mFaces.size(); ++lFaceCounter)
	{
     if(pMap.mFaces[lFaceCounter].mType == 1)
      {
       /*
		fprintf(pFile, "Face %d\n", lFaceCounter);
       ...
       */
       ...
       //fprintf(pFile, "%d ", pMap.mFaces[lFaceCounter].mNbMeshVertices);
       //fprintf(pFile, "%f %f %f ", pMap.mFaces[lFaceCounter].mNormal[0], pMap.mFaces[lFaceCounter].mNormal[1], pMap.mFaces[lFaceCounter].mNormal[2]);
		
		/*
        for(i = 0; i < pMap.mFaces[lFaceCounter].mNbMeshVertices; i++)
		 {
		  real_vind = pMap.mFaces[lFaceCounter].mVertex;
		  real_vind += pMap.mMeshVertices[pMap.mFaces[lFaceCounter].mMeshVertex + i].mMeshVert;
		  fprintf(pFile, "%d ", real_vind);
         }
    */

        for(i = 0; i < pMap.mFaces[lFaceCounter].mNbMeshVertices / 3; i++)
		 {
		  tri_a = pMap.mFaces[lFaceCounter].mVertex;
		  tri_a += pMap.mMeshVertices[pMap.mFaces[lFaceCounter].mMeshVertex + i * 3].mMeshVert;

          tri_b = pMap.mFaces[lFaceCounter].mVertex;
		  tri_b += pMap.mMeshVertices[pMap.mFaces[lFaceCounter].mMeshVertex + i * 3 + 1].mMeshVert;

          tri_c = pMap.mFaces[lFaceCounter].mVertex;
		  tri_c += pMap.mMeshVertices[pMap.mFaces[lFaceCounter].mMeshVertex + i * 3 + 2].mMeshVert;
          
          fprintf(pFile, "%d ", 3);
          fprintf(pFile, "%d ", tri_a);
          fprintf(pFile, "%d ", tri_b);
          fprintf(pFile, "%d ", tri_c);
         }
