...
void copy_vec3f(VEC3F dest, VEC3F source)
{
 dest[0] = source[0];
 dest[1] = source[1];
 dest[2] = source[2];
...
void transform_vec3f(VEC3F dest, MAT16F mat)
{
 VEC3F source;
 copy_vec3f(source, dest);

 dest[0] = source[0] * mat[0][0] +
           source[1] * mat[1][0] +
           source[2] * mat[2][0] +
                       mat[3][0];
 dest[1] = source[0] * mat[0][1] +
           source[1] * mat[1][1] +
           source[2] * mat[2][1] +
                       mat[3][1];
 dest[2] = source[0] * mat[0][2] +
           source[1] * mat[1][2] +
           source[2] * mat[2][2] +
                       mat[3][2];
}
...
void rotate_mat16f(MAT16F matrix, float ax, float ay, float az)
{
 ...
 xmat[0][0] = 1.0; xmat[0][1] = 0.0;      xmat[0][2] = 0;       xmat[0][3] = 0.0;
 ...
 mat16f_product(mat1, matrix, xmat);
 mat16f_product(mat2, mat1, ymat);
 mat16f_product(matrix, mat2, zmat);
}
