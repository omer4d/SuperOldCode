...
int main()
{
 ...
 map_tri[snum * 2].tex[1] = vec2f(20.0, 0.0); 
 map_tri[snum * 2].tex[2] = vec2f(20.0, 20.0);

 map_tri[snum * 2 + 1].c = vnum * 2 + 2; map_tri[snum * 2 + 1].b = vnum * 2 + 3; map_tri[snum * 2 + 1].a = vnum * 2;
 map_tri[snum * 2 + 1].tex[0] = vec2f(20.0, 20.0);
 map_tri[snum * 2 + 1].tex[1] = vec2f(0.0, 20.0);
 map_tri[snum * 2 + 1].tex[2] = vec2f(0.0, 0.0);

 int tri_num = snum * 2 + 2;

 BITMAP **lightmap = (BITMAP**)malloc(tri_num * sizeof(BITMAP *));
 for(i = 0; i < tri_num; i++)
  lightmap[i] = create_bitmap(16, 16);
   ...
   for(i = 0; i < snum * 2 + 2; i++)
    {
     ...
     map_tri[i].n = NORMALIZED_VEC3F(map_tri[i].n);
