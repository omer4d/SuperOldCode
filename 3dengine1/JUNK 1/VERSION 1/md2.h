...
void convert_md2_to_base(VERTEX **model_vertex, TRI **model_tri, MD2_MODEL *mdl, float scale)
{
 int i;
 ...
 *model_tri = (TRI *)malloc(sizeof(TRI) * mdl->header.num_tris);

 for(i = 0; i < mdl->header.num_vertices; i++)
  {
   pvert = &pframe->vert[i];
   (*model_vertex)[i].local = vec3f((pvert->v[0] * pframe->scale[0] + pframe->translate[0]) * scale,
 ...
 for(i = 0; i < mdl->header.num_tris; i++)
  {
   (*model_tri)[i].a = mdl->triangle[i].index_xyz[0];
   (*model_tri)[i].b = mdl->triangle[i].index_xyz[1];
   (*model_tri)[i].c = mdl->triangle[i].index_xyz[2];
   (*model_tri)[i].tex[0].x = (float)mdl->texcoord[mdl->triangle[i].index_st[0]].s / (float)mdl->header.skinwidth;
   (*model_tri)[i].tex[0].y = (float)mdl->texcoord[mdl->triangle[i].index_st[0]].t / (float)mdl->header.skinheight;
   (*model_tri)[i].tex[1].x = (float)mdl->texcoord[mdl->triangle[i].index_st[1]].s / (float)mdl->header.skinwidth;
   (*model_tri)[i].tex[1].y = (float)mdl->texcoord[mdl->triangle[i].index_st[1]].t / (float)mdl->header.skinheight;
   (*model_tri)[i].tex[2].x = (float)mdl->texcoord[mdl->triangle[i].index_st[2]].s / (float)mdl->header.skinwidth;
   (*model_tri)[i].tex[2].y = (float)mdl->texcoord[mdl->triangle[i].index_st[2]].t / (float)mdl->header.skinheight;
  }
}
...
void convert_md2_frame_to_base(VERTEX *model_vertex, MD2_MODEL *mdl, float n, float scale)
{
 ...
 MD2_FRAME *pframe1 = &mdl->frame[wrap(f1, 0, mdl->header.num_frames)];
 MD2_FRAME *pframe2 = &mdl->frame[wrap(f2, 0, mdl->header.num_frames)];
 MD2_VERTEX *pvert1;
 MD2_VERTEX *pvert2;
 VEC3F rv1, rv2;
 ...
 for(i = 0; i < mdl->header.num_vertices; i++)
  {
   ...
   model_vertex[i].local = VEC3F_BLEND(rv1, rv2, (1.0 - k));
  }
}
