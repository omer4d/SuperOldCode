...
void draw_grid()
{
 ...
 float fr = r / 30.0, fg = g / 30.0, fb = b / 30.0;
 float r2, g2, b2;
 r2 = 0, g2 = 0, b2 = 0;
 ...
 for(z = 0; z < 30; z++)
  {
   ...
   GFX_TZ = t + 20.0 * (1.0 - z);
   for(x = BORDER_MINX - 1.0; x < BORDER_MAXX + 1.0; x += 0.5)
    vector_line(vector(x, BORDER_MINY), vector(x, BORDER_MAXY));
   for(y = BORDER_MINY - 1.0; y < BORDER_MAXY + 1.0; y += 0.5)
...
int main()
{
 ...
 //p2.body[25].inverse_mass = 1.0 / 100000000.0;
 ...
 while(!exit)
  {
   ...
   VECTOR spring = USCALE_VECTOR(VECTOR_DIFF(p1.body[25].pos, p2.body[25].pos), 900.0);
   p2.body[25].force = VECTOR_SUM(p2.body[25].force, spring);
   p1.body[25].force = VECTOR_DIFF(p1.body[25].force, spring);
  }
