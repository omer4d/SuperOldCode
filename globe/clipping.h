...
int scr_clip(int curr, REND_INT_POINT *v0, REND_INT_POINT *v1)
{
 ...
 if(flag == 1)
  {
   float dz = (v0->affine_interp[2] - v1->affine_interp[2]) / (v0->affine_interp[2] * v1->affine_interp[2]);
   ...
   if(fabs(dz) > AFFINE_EPSILON) k = (v0->affine_interp[2] - out[i].affine_interp[2]) /
                                     (v0->affine_interp[2] * out[i].affine_interp[2] * dz);

   for(j = 0; j < MAX_PERSPC_INTERP; j++)
    out[i].perspc_interp[j] = v0->perspc_interp[j] + k * (v1->perspc_interp[j] - v0->perspc_interp[j]);
 ...
 if(flag == 1)
  {
   ...
   if(fabs(v1->screen.y - v0->screen.y) > FLOAT_EPSILON)
    k = (out[i].screen.y - v0->screen.y) / (v1->screen.y - v0->screen.y);
   else if(fabs(v1->screen.x - v0->screen.x) > FLOAT_EPSILON)
    k = (out[i].screen.x - v0->screen.x) / (v1->screen.x - v0->screen.x);

   for(j = 0; j < MAX_AFFINE_INTERP - 2; j++)
    out[i].affine_interp[j] = v0->affine_interp[j] + k * (v1->affine_interp[j] - v0->affine_interp[j]);
