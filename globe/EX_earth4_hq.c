...
BITMAP *buffer;
BITMAP *colormap, *cloudmap, *lightmap, *normalmap;
 ...
 for(x = minx; x < maxx + 1; x++)
  {
   ...
   float fu = u * colormap->w, fv = v * colormap->h;

   VEC3F n, p;
   int nc = sample_4(normalmap, fu, fv, linear_interp);//RI_TEXEL(normalmap, iu, iv);
   ...
   float cloud_br = VEC3F_DOT_PRODUCT(p, local_light_dir);
   if(br < 0.0) { br = 0.0; }
   if(cloud_br < 0.0) { cloud_br = 0.0; }

   col = sample_4(colormap, fu, fv, linear_interp);
   r = getr32(col) * br;
   g = getg32(col) * br;
   b = getb32(col) * br;

   br = 1.0 - br;
   float lm = getb32(sample_4(lightmap, fu, fv, linear_interp)) * 0.00392156 * br * br * br * br;

   k = getb32(sample_4(cloudmap, fu + scroll, fv, linear_interp)) * 0.00392156 * cloud_br;
...
void init()
{
 ...
 lightmap = create_bitmap(colormap->w, colormap->h);

 for(j = 0; j < cloudmap->h; j++)
  for(i = 0; i < cloudmap->w; i++)
   {
    int alpha = geta32(_getpixel32(colormap, i, j));
    putpixel(cloudmap, i, j, alpha);

    alpha = geta32(_getpixel32(normalmap, i, j));
    putpixel(lightmap, i, j, alpha);
   }
