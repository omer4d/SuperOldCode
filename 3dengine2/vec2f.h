...
float angle_between_vec2f_and_up(VEC2F c, VEC2F v)
{
 VEC2F cv = NORMALIZED_VEC2F(VEC2F_DIFF(v, c)), up = vec2f(0.0, 1.0);

 if(cv.x >= 0.0)
  return acos(VEC2F_DOT_PRODUCT(cv, up));
 return (2.0 * M_PI - 0.0000001) - acos(VEC2F_DOT_PRODUCT(cv, up));
}
