...
void get_z(VEC3F n, float D, float f, float mx, float my, float kx, float ky, float sx, float sy, VEC3F *v)
{
 float k = 1.0 / (n.x * my * (sx - kx) + n.y * mx * (sy - ky) + n.z * f * mx * my);
...
void init()
{
 ...
 setup_projection(90.0, -1.333, 1.333, -1.0, 1.0, 0.01, 200.0);
 ...
 while(!exit_flag)
  {
   ...
   clear_bitmap(buffer);
   clear_to_color(BASE_INT_z_buffer, 0.0);
