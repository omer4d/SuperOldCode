...
void init()
{
 ...
 //setup_projection(90.0, -1.0 * (float)SCREEN_W / (float)SCREEN_H, 1.0 * (float)SCREEN_W / (float)SCREEN_H, -1.0, 1.0, 1.0, 1000.0);
...
{
 int exit = 0, i;
 VEC3F pos = vec3f(0.0, 0.0, 10.0), ang = ZERO_VEC3F;
 MAT16F mat, sky_mat;
 ...
 BITMAP *skybox[6];
 skybox[0] = load_bitmap("data/texture.bmp", NULL);
 skybox[1] = load_bitmap("data/texture.bmp", NULL);
 skybox[2] = load_bitmap("data/texture.bmp", NULL);
 skybox[3] = load_bitmap("data/texture.bmp", NULL);
 skybox[4] = load_bitmap("data/texture.bmp", NULL);
 skybox[5] = load_bitmap("data/texture.bmp", NULL);
 ...
 VEC3F cam_pos = vec3f(0.0, 0.0, -100.0), cam_dir, cam_dir_normal, cam_ang = vec3f(0.0, 0.0, 0.0);
 ...
 while(!exit)
  {
   ...
   translate_mat16f(sky_mat, -cam_pos.x, -cam_pos.y, -cam_pos.z);
